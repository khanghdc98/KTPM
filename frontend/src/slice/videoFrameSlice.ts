...
export const setVideoSource = createAsyncThunk("videoFrame/setVideoSource", async (videoSrc: string) => {
    return videoSrc;
});
...
export const fetchFrame = createAsyncThunk(
    ...
    async (timestamp: number, { getState }) => {
        ...
        return new Promise<{ timestamp: number; frameData: string }>((resolve, reject) => {
...
export const videoFrameSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(setVideoSource.fulfilled, (state, action: PayloadAction<string>) => {
                state.videoSrc = action.payload;
                state.cache = {};
            })
            .addCase(fetchFrame.fulfilled, (state, action: PayloadAction<{ timestamp: number; frameData: string }>) => {
