...
import { Detection, GeneralData } from "../types/ResultType";
...
export const Layout1Tab = ({ data }: { data: GeneralData }) => {
    ...
    const rowRenderer = ({
        ...
        const fameTimestamps = Object.keys(timeStampAndDetections).map((timestamp) => Number.parseInt(timestamp));
        const tokens = Object.values(timeStampAndDetections).flatMap((detection: Detection[]) => {
        ...
        return (
            <CellMeasurer
                ...
                parent={parent}
            >
                <Box style={style} display="flex" alignItems="center" padding="10px" borderBottom="1px solid #ddd">
                    <Box display="flex" flexDirection="row" overflow="auto" width="60%" sx={{ overflowX: "auto", overflowY: "hidden" }}>
